...
// implements 可实现继承 通过继承可以直接继承父元素属性 这种方式很适合父类不止一个的情况下，
// 同时可更改类的prototype属性 可以改变他的所有实例
...
const generateArray = (table: HTMLElement) => {
  ...
  for (let R = 0; R < rows.length; ++R) {
    ...
    for (let C = 0; C < columns.length; ++C) {
      ...
      const collspanStr = cell.getAttribute("colspan");
      const rowspanStr = cell.getAttribute("rowspan");
      let colspan;
      let rowspan;
      if (collspanStr) {
        colspan = parseInt(collspanStr);
      }
      if (rowspanStr) {
